        ...
        Returns:
            List of dictionaries with source and content information
        """
        context_metadata = []
        for doc in context_docs:
            # Get file name from metadata
            source = doc.meta.get("file_name", "Unknown source")

            # Create a snippet from the content
            max_snippet_length = 500
            snippet = doc.content[:max_snippet_length]
            if len(doc.content) > max_snippet_length:
                snippet += "..."

            # Create context item with source, content, and file_name attributes
            context_item = {
                "source": f"Content from {source}",
                "snippet": snippet,
                "file_name": source,
            }
            context_metadata.append(context_item)

        # If no context documents, add a note that the response is generated from model knowledge
        if not context_docs:
            context_metadata.append(
                {
                    "source": "Generated by Llama 2 Model",
                    "snippet": "This response was generated using the model's pre-trained knowledge as no relevant documents were found.",
    ...
    def save_uploaded_file(
        ...
        file_path = self.document_store_dir / unique_filename
...
"""
            ...
            # Check if the answer itself appears garbled, with special handling for model output
            if self._detect_garbled_text(answer, is_model_output=True):
                logger.warning(
                    "Generated answer appears to be garbled, returning error response"
                )
                answer = "I apologize, but I'm having trouble generating a clear response. Please try reformulating your question."

            # Add attribution to model response
            model_name = "Llama 2"
            if document_issue_notice:
                source_label = f"Generated by {model_name} (document processing issues)"
                ...
                source_label = f"Generated by {model_name}"
                notice = "This response was generated using the model's knowledge as no relevant documents were found."

            # Add model attribution to the answer text
            if not answer.endswith(f"[Generated by {model_name}]"):
            ...
            response = {
                ...
                "answer": answer,
                "context": [
                    {"source": source_label, "content": notice, "file_name": model_name}
                ],
            }
            ...
            return {
                ...
                "context": [
                    {
                        "source": f"Error (Generated by {model_name})",
    ...
    def _create_knowledge_prompt(
        ...
        """Create a prompt for knowledge-based generation without context for Llama 2."""
...
responses using the fine-tuned Phi-3 Medium model.
"""
